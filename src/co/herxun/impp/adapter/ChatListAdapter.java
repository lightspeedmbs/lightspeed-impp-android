            ...
            for (Chat chat : chatList) {
                if (chat.topic != null && chat.topic.topicName.contains(charText)) {
                    ...
                    if (userMap.containsKey(chat.targetClientId)
                            && userMap.get(chat.targetClientId).userName.contains(charText)) {
                        filteredChatList.add(chat);
                    }
                }
            }
    ...
    public class ChatListItem extends RelativeLayout {
        ...
        public void setData(Chat chat) {
            if (chat.topic != null) {
                ...
                Room room = roomManager.isRoomExists(chat.topic.topicId);
                if (room != null && room.topicId != null) {
                    ImageLoader.getInstance(ct).DisplayImage(room.photoUrl, imgIcon, R.drawable.friend_group, true);
                } else {
                    imgIcon.setImageResource(R.drawable.friend_group);
                }
            } else if (chat.targetClientId != null) {
                if (userMap.containsKey(chat.targetClientId)) {
                    if (userMap.get(chat.targetClientId) != null) {
                        textName.setText(userMap.get(chat.targetClientId).userName);
                        ImageLoader.getInstance(ct).DisplayImage(userMap.get(chat.targetClientId).userPhotoUrl,
                                imgIcon, R.drawable.friend_default, true);
                    }
                }
            }
