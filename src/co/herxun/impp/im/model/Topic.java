...
public class Topic extends Model implements Serializable {
    ...
    public Topic update() {
        Topic userExisit = new Select().from(Topic.class).where("topicId = ? ", topicId).executeSingle();
        // 不存在
        if (userExisit == null) {
            save();
            return this;
    ...
    public void removeMember(String clientId) {
        new Delete().from(TopicMember.class).where("topicId = \"" + topicId + "\" and clientId = \"" + clientId + "\"")
                .executeSingle();
    }
    ...
    public boolean hasMember(String clientId) {
        List<TopicMember> topicMembers = members();
        for (TopicMember member : topicMembers) {
