...
public class UserDetailActivity extends BaseActivity implements Observer {
    ...
    private void initView() {
        ...
        if (!user.clientId.equals(UserManager.getInstance(this).getCurrentUser().clientId)) {
            if (UserManager.getInstance(this).getCurrentUser().isFriend(user.clientId)) {
                ...
                mUserDetailView.getTextButton().setVisibility(View.VISIBLE);
                mUserDetailView.setButton(getString(R.string.friend_request_status_add), new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        DBug.e(user.userName, user.clientId);
                        mUserDetailView.setButton(getString(R.string.friend_request_status_requesting), null);

                        UserManager.getInstance(UserDetailActivity.this).sendFriendRequest(user,
                                new IAnSocialCallback() {
                                    ...
                                    public void onSuccess(JSONObject arg0) {
                                        mUserDetailView.setButton(getString(R.string.friend_request_status_sent), null);
                                    }
                                });
